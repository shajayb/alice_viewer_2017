...
{
	Eigen::SparseMatrix<Cdouble> C_sub(nodes.size(), C.cols());
	C_sub.transpose();
	C.transpose();
...
void setup()
{
	...
	S.sliders[S.numSliders - 1].maxVal = 50;
	...
	B.addButton(&drawFreeVectors, " freeVectors");
...
//	drawGrid(20.0);
	...
	glLineWidth(1);
	glColor3f(0, 0, 0);
	if (drawFreeVectors)for (int i = 0; i < Ui.rows(); i++) drawLine(face_centers[i], face_centers[i] + fieldVectors[i]);

	glLineWidth(5);
	glColor3f(1, 0, 0);
	for (int i = Ui.rows(); i < fieldVectors.size(); i++) drawLine(face_centers[i], face_centers[i] + fieldVectors[i] * 0.5);

	glPointSize(1);
	glColor3f(0, 0, 0);
	if(drawFreeVectors)for (int i = 0; i < fieldVectors.size(); i++) drawPoint(face_centers[i]);
